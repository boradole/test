...
public class UpdateQueue {
	...
	private static Logger log = LoggerFactory.getLogger(UpdateQueue.class);
	...
	@Autowired
	public UpdateQueue(final MessageSourceAccessor msAcc) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				int update_Limit = Integer.valueOf(msAcc.getMessage("DB_LIMIT"));
				...
				for (;;) {
					try {
						...
						for (; i < update_Limit; i++) {
							...
							if (val == null) {
								queueSercie.add(new QueueInterface("updateQueue") {
									@Override
									public void doAsyncProcess() {
										connectService.modifyAuthForBatch(updateMap);
									}
								});
								break;
							}
						...
						if (i > 0)
							log.debug("[UpdateQueue] poll Count : {} ", i + 1);
						if (interrupted) {
